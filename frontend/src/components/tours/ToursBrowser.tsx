  ...
  return (
    <div className="min-h-screen bg-[#f8f6f1] pb-24 dark:bg-dark">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {!loading && !error && <div className="grid gap-7 md:grid-cols-2 lg:grid-cols-3">{tours.map((tour) => <article key={tour.id} className="overflow-hidden rounded-3xl border bg-white shadow-sm dark:border-white/10 dark:bg-white/5"><div className="relative h-56">{tour.image ? <ResponsivePhoto src={tour.image} alt={tour.media?.[0]?.altText || tour.title} /> : <div className="absolute inset-0 bg-stone-200" />}<span className="absolute left-4 top-4 rounded-full bg-white/95 px-3 py-1 text-xs font-bold text-stone-800">{tour.category.name}</span></div><div className="p-6"><h2 className="font-body text-2xl font-extrabold leading-8 text-stone-900 dark:text-white">{tour.title}</h2><p className="mt-3 line-clamp-2 text-sm leading-6 text-stone-600 dark:text-white/55">{tour.shortDesc}</p><div className="mt-4 flex gap-5 text-xs text-stone-500"><span className="flex gap-1"><Clock3 className="h-4 w-4" />{tour.duration}</span><span className="flex gap-1"><MapPin className="h-4 w-4" />{t.toursPage.pickupAvailable}</span></div><div className="mt-6 flex items-end justify-between border-t pt-5 dark:border-white/10"><TourCardPrice tour={tour} fromLabel={t.toursPage.from} /><Link href={href(`/tours/${tour.slug}`)} className="rounded-xl bg-[#123f35] px-5 py-3 text-sm font-extrabold text-white">{t.toursPage.viewDetails}</Link></div></div></article>)}</div>}
